...
const checkSpeciesValues = async (): Promise<void> => {
  ...
  try {
    ...
    const uniqueSpecies = [...new Set(data.map((animal: AnimalSpecies) => animal.species))];
    console.log('✅ Found the following species values:');
    uniqueSpecies.forEach((species: string) => {
    ...
    const categoriesToTest: string[] = ['Cat', 'Dog', 'Bird', 'Hamster'];
