...
export const optimizeImage = async (
  uri: string,
  { width = 600, height, quality = 0.7, format = 'jpeg' }: OptimizeImageOptions = {}
): Promise<string> => {
  try {
    const manipulatorOptions = {
      compress: quality,
      format: ImageManipulator.SaveFormat[format.toUpperCase() as 'JPEG' | 'PNG'],
    };
...
export const cacheImage = async (uri: string): Promise<string> => {
  try {
    ...
    metadata.files[filename] = {
      lastAccessed: Date.now(),
      size: parseInt(downloadResult.headers['content-length'] || '0', 10),
      source: uri
    };
    ...
    if (Date.now() - metadata.lastCleanup > ONE_DAY) {
      cleanupImageCache(metadata, cacheDir).catch(e => console.error(e));
    }
 ...
 */
export const cleanupImageCache = async (metadata: CacheMetadata, cacheDir: string): Promise<void> => {
  try {
    ...
    const totalSize = files.reduce((sum, [_, info]) => sum + (info.size || 0), 0);
 ...
 */
export const getFlashListConfig = (contentType: 'images' | 'text' | 'mixed' = 'mixed'): FlashListConfig => {
  // Base configuration
  const config: FlashListConfig = {
 ...
 */
export const getAdaptiveQuality = async (): Promise<QualitySettings> => {
 ...
 */
export const getProgressiveSources = (originalUrl: string): ProgressiveSources => {
  return {
