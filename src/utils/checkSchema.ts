...
interface Animal {
  id: string;
  [key: string]: any; // We're checking the schema dynamically, so we don't know all fields
}
...
const checkAnimalSchema = async (): Promise<void> => {
  ...
  try {
    ...
    const sampleAnimal = data[0] as Animal;
