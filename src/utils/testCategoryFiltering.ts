...
const testCategoryFiltering = async (): Promise<void> => {
  // List of categories to test
  const categoriesToTest: ('all' | 'cat' | 'dog' | 'bird' | 'other')[] = ['all', 'cat', 'dog', 'bird', 'other'];
  ...
  for (const category of categoriesToTest) {
    ...
    if (data && data.length > 0) {
      ...
      data.slice(0, 3).forEach((animal: Animal) => {
