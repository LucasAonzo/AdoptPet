...
const DEV_SUPABASE_URL = "https://your-project.supabase.co";
const DEV_SUPABASE_ANON_KEY = "your-anon-key";

// Get Supabase URL and anon key from environment variables or constants
const supabaseUrl = Constants.expoConfig?.extra?.supabaseUrl || process.env.SUPABASE_URL || DEV_SUPABASE_URL;
const supabaseAnonKey = Constants.expoConfig?.extra?.supabaseAnonKey || process.env.SUPABASE_ANON_KEY || DEV_SUPABASE_ANON_KEY;
...
if (!supabaseUrl || !supabaseAnonKey) {
  ...
  const errorMessage = `Missing Supabase configuration: ${missingKeys.join(', ')}`;
